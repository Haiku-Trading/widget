 ...
 * @param allowedTokenIids Optional whitelist of token iids - only tokens in this list will be returned
...
export function resolveTokensFromIids(
  ...
  for (const iid of iids) {
    ...
    if (token) {
      ...
      // Check if token is in the whitelist (if a whitelist is provided)
      if (allowedTokenIids && allowedTokenIids.length > 0) {
        if (!allowedTokenIids.some(
          allowed => allowed.toLowerCase() === token.iid.toLowerCase()
        )) {
          console.warn(`Token ${iid} is not in the allowed token whitelist`)
          continue
        }
      }
...
export function resolveTokenFromIid(
  ...
  const resolved = resolveTokensFromIids([iid], tokenData, hiddenChains, hiddenProtocols, allowedTokenIids)
