...
export function generatePaletteCSS(palette: ColorPalette): Record<string, string> {
    ...
    if (palette.backgroundColor && isValidColor(palette.backgroundColor)) {
        cssVars["--bg-surface"] = colorToHsl(palette.backgroundColor);
        cssVars["--bg-primary"] = colorToHsl(palette.backgroundColor);
        cssVars["--background"] = colorToHsl(palette.backgroundColor);
    }

    // Handle button color
    if (palette.button && isValidColor(palette.button)) {
        cssVars["--button-bg"] = colorToHsl(palette.button);
        cssVars["--button-text"] = getContrastColor(palette.button);
        // Create darker hover state
    ...
    if (palette.borderColor && isValidColor(palette.borderColor)) {
        cssVars["--border"] = colorToHsl(palette.borderColor);
        cssVars["--stroke-grey-primary"] = colorToHsl(palette.borderColor);
        cssVars["--stroke-grey-secondary"] = colorToHsl(palette.borderColor);
    }

    // Handle muted background color
    if (palette.mutedBackground && isValidColor(palette.mutedBackground)) {
        cssVars["--bg-section"] = colorToHsl(palette.mutedBackground);
    }

    // Handle text colors
    if (palette.primaryText && isValidColor(palette.primaryText)) {
        cssVars['--foreground'] = colorToHsl(palette.primaryText)
    }
    if (palette.secondaryText && isValidColor(palette.secondaryText)) {
        cssVars['--text-grey-secondary'] = colorToHsl(palette.secondaryText)
    }
